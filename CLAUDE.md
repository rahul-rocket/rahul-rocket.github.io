...
- `apps/web` — the Next.js 14 App Router site. Alias `@/*` points at the app root.
...
- Tailwind's `content` globs in `apps/web/tailwind.config.ts` already cover `packages/ui/src`.
  If a class only appears in the UI package, it is still scanned.
